...
class IduNavigator:
        ...
        try:
            ...
            for iframe in iframes:
                ...
                if src and "chat_valorizacion/chat" in src:
                    logger.info("Iframe del chat encontrado. Cambiando al iframe...")
    ...
    def buscar_tabla_formulario(self, timeout=15):
        try:
            ...
            logger.info("Tabla con class='form' encontrada")
            return tabla_form
    ...
    def buscar_input_username(self, tabla_form=None, timeout=15):
        logger.info("Buscando el input para colocar el nombre de usuario...")
        ...
        try:
            ...
            logger.info("Input de usuario encontrado")
    ...
    def preparar_formulario(self, timeout=15):
        ...
        if not tabla_form:
            logger.warning("No se puede continuar sin la tabla del formulario")
            return None, None, en_iframe_chat

        input_username = self.buscar_input_username(tabla_form, timeout)

        if input_username:
            logger.info("Formulario preparado exitosamente")
        else:
            logger.warning("Formulario encontrado pero sin input de usuario")
    ...
    def click_iniciar_chat(self, timeout=15):
        try:
            ...
            time.sleep(2)
            logger.info("Click en 'Iniciar Chat' completado exitosamente")
            return True
        ...
        for intento in range(max_intentos):
            try:
                ...
                if intento < max_intentos - 1:
                    ...
                    logger.error("No se pudo reiniciar el navegador después de 3 intentos")
                    raise